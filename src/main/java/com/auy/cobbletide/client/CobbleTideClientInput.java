...
public final class CobbleTideClientInput {
    ...
    public static void onClientTick(ClientTickEvent.Post event) {
        ...
        boolean active = CatchMinigameOverlay.isActive();
        if (!active) {
            reset();
            return;
        }
        if (!minigameSeen) {
            ...
            CobbleTide.LOGGER.debug("CobbleTide minigame opened - waiting for right-click release");
            return;
        }
        if (waitingForRelease && !minecraft.options.keyUse.isDown()) {
            waitingForRelease = false;
            CobbleTide.LOGGER.debug("CobbleTide minigame armed");
        }
    }
    @SubscribeEvent
    public static void onInteraction(InputEvent.InteractionKeyMappingTriggered event) {
        ...
        if (!(player.fishing instanceof PokeRodFishingBobberEntity)) {
            return;
        }
        event.setSwingHand(false);
        event.setCanceled(true);
        if (!minigameSeen || waitingForRelease) {
            CobbleTide.LOGGER.debug("Ignoring original Poké Rod reel input");
            return;
        }
        CobbleTide.LOGGER.debug("Tide minigame interaction accepted");
