...
public final class CobbleTideConfig {
    ...
    static {
        ...
        INSTANCE = new CobbleTideConfig(builder);
        SPEC = builder.build();
    }

    private CobbleTideConfig(ModConfigSpec.Builder builder) {
        builder.comment(
        ...
        onePerfectIvChance = builder.defineInRange("one_iv_percent", 6.0, 0.0, 100.0);
        twoPerfectIvChance = builder.defineInRange("two_iv_percent", 3.0, 0.0, 100.0);
        threePerfectIvChance = builder.defineInRange("three_iv_percent", 1.5, 0.0, 100.0);
        fourPerfectIvChance = builder.defineInRange("four_iv_percent", 0.5, 0.0, 100.0);
        fivePerfectIvChance = builder.defineInRange("five_iv_percent", 0.1, 0.0, 100.0);
        sixPerfectIvChance = builder.defineInRange("six_iv_percent", 0.02, 0.0, 100.0);
        builder.pop();
    }

    public int getShinyDenominator(String rarity) {
        String normalized = rarity == null ? "common" : rarity.trim().toLowerCase(Locale.ROOT);
        return switch (normalized) { case "uncommon" -> uncommonShinyDenominator.get();
            case "rare" ->
